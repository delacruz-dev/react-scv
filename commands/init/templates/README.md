...
instead relying on npm scripts. The build process is managed via workshare Scv webpack
...
Scv uses Webpack and Webpack Dev Server as the build engine to transpile code,
...
pass this configuration file to the `npm scripts`. Scv exposes a number of
...
```js
// webpack.start.js
let devConfig = require('scv/config/webpack.dev');
...
{
  "scripts": {
    "start": "scv start --port <%= data.port %> --config webpack.start.js"
  }
}
```
